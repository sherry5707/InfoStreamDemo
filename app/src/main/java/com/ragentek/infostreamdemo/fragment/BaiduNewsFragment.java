...
public class BaiduNewsFragment extends Fragment {
    ...
    private BaiduNewsAdapter baiduAdapter;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        if (engine == null) {
            engine = BaiduEngine.GetInstance(context);
        }
        if (channel == null) {
            channel = this.getArguments().getString("channel");
        }
        baiduAdapter = engine.createBaiduNewsAdapter(channel);
        if (activity != null)
            //adapter.setActivity(activity);
            baiduAdapter.setActivity(activity);;
        ...
        listView.setAdapter(baiduAdapter);
    ...
    public void retrieveBaiduNews() {
        refresh_layout.setRefreshing(true);
        new Thread(new Runnable() {
            @Override
            public void run() {
                final ArrayList<BaiduItem> newsList = new ArrayList<BaiduItem>(engine.refreshBaiduData(baiduAdapter));
                if (newsList != null) {
                    ((Activity) context).runOnUiThread(new Runnable() {
                        public void run() {
                            refresh_layout.setRefreshing(false);
                            for (BaiduItem entity : newsList) {
                                baiduAdapter.addItem(entity);
                            }
                            baiduAdapter.notifyDataSetChanged();
                            listView.setSelection(0);
                        }
                    });
                }
    ...
    public void retrieveBaiduNews2() {
        refresh_layout.setRefreshing(true);
        new Thread(new Runnable() {
            @Override
            public void run() {
