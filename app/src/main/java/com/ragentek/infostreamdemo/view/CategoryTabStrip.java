package com.ragentek.infostreamdemo.view;

import java.util.ArrayList;

import com.ragentek.infostreamdemo.R;
import com.ragentek.infostreamdemo.TextDrawable;
import com.ragentek.infostreamdemo.R.color;
import com.ragentek.infostreamdemo.R.drawable;
import com.ragentek.infostreamdemo.R.id;
import com.ragentek.infostreamdemo.R.layout;
import com.ragentek.infostreamdemo.bean.ChannelItem;
import com.ragentek.infostreamdemo.tool.DataTools;
...
public class CategoryTabStrip extends HorizontalScrollView {
	...
	public CategoryTabStrip(Context context, AttributeSet attrs, int defStyle) {
		...
		scrollOffset = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, scrollOffset, dm);		//这个方法是转变为标准尺寸的一个函数dp->px
		
		defaultTabLayoutParams = new LinearLayout.LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.MATCH_PARENT);

		// 绘制高亮区域作为滑动分页指示器
		indicator = getResources().getDrawable(drawable.bg_category_indicator);
		// 左右边界阴影效果
		left_edge = getResources().getDrawable(drawable.ic_category_left_edge);
        right_edge = getResources().getDrawable(drawable.ic_category_right_edge);
	}
	...
	private void addTab(final int position, String title) {
		ViewGroup tab = (ViewGroup)mLayoutInflater.inflate(layout.category_tab, this, false);
		TextView category_text = (TextView) tab.findViewById(id.category_text);
		...
		category_text.setTextColor(getResources().getColor(color.category_tab_text));
		category_text.setTextSize(TypedValue.COMPLEX_UNIT_DIP,16);
	...
	private void calculateIndicatorRect(Rect rect) {
		...
		TextView category_text = (TextView) currentTab.findViewById(id.category_text);
        ...
        if (currentPositionOffset > 0f && currentPosition < tabCount - 1) {
			...
			TextView next_category_text = (TextView) nextTab.findViewById(id.category_text);
...
*/        
        rect.set(((int) left)+getPaddingLeft()-20 , getPaddingTop() + currentTab.getTop() + category_text.getTop(),
 				((int) width)+ getPaddingLeft()+20, currentTab.getTop() + getPaddingTop() + category_text.getTop() + category_text.getHeight());
...
/*		ViewGroup tab = (ViewGroup) tabsContainer.getChildAt(currentPosition);
		if (tab != null) {
			TextView category_text = (TextView) tab.findViewById(R.id.category_text);
			
			category_text.setTextColor(getResources().getColor(R.color.category_tab_highlight_text));
			category_text.setTextSize(TypedValue.COMPLEX_UNIT_DIP,18);
		}
		for (int j = 0; j < tabsContainer.getChildCount(); j++) {
			tab = (ViewGroup) tabsContainer.getChildAt(j);
	...
	public void draw(Canvas canvas) {
        ...
        while (i < tabsContainer.getChildCount()) {
            if (i < currentPosition - 1 || i > currentPosition + 1) {
                ...
                if (category_text != null) {
                    ...
                    textDrawable.setTextSize(TypedValue.COMPLEX_UNIT_DIP,18);
                    textDrawable.setTextColor(getResources().getColor(color.category_tab_highlight_text));
        ...
        int width = getWidth();
        int maxScrollx=getChildAt(0).getWidth() * getChildCount() - getWidth();
        canvas.translate((float) top, 0.0f);
        if (left_edge == null || top <= 0) {
            if (right_edge == null || top >= getScrollRange()) {
                canvas.restoreToCount(i);
            }
            if (maxScrollx > 0 && getScrollX() < maxScrollx){
            	
            	right_edge.setBounds(width - right_edge.getIntrinsicWidth(), 0, width, height-20);
            	right_edge.draw(canvas);
            	canvas.restoreToCount(i);
            }
        }
        if(getScrollX()>0){
        	left_edge.setBounds(0, 0, left_edge.getIntrinsicWidth(), height-20);
		...
		if (maxScrollx > 0 && getScrollX() < maxScrollx) { // 如果scrollX滑动的距离<计算出来的最大scrollX距离
...
/*			ViewGroup tab = (ViewGroup) tabsContainer.getChildAt(currentPosition);
			...
			if (tab != null) {
				...
				TextView category_text = (TextView) tab.findViewById(R.id.category_text);
				category_text.setTextColor(getResources().getColor(R.color.category_tab_highlight_bg));
			}
